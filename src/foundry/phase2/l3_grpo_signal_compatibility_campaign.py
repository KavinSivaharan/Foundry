"""Run and publish four exact signal-qualified official compatibility smokes."""
...
OUTPUT_NAME = "milestone14b_r1_compatibility.json"
...
def _runtime_command(
    ...
    *,
    ...
    return [
        str(training_python),
        "-m",
        "foundry.phase2.l3_grpo_signal_compatibility_runtime",
        "--root",
        str(root),
        "--arm",
        arm,
...
def _validated_run(
    *,
    ...
    _verify(raw, "raw_evidence_sha256")
    _verify(partial, "partial_evidence_sha256")
    _verify(envelope, "envelope_sha256")
    ...
    if (
        ...
        or cast(int, gate.get("nonzero_variance_group_count", 0)) < 1
        or cast(int, gate.get("nonzero_gradient_group_count", 0)) < 1
        or cast(int, gate.get("policy_update_count", 0)) < 1
        ...
        or packet.get("source_kinds") != ["task", "base_replay"]
...
def build_compatibility_result(
    *,
    ...
    selection = _read(root / "results/phase2_vetted_corpus" / SELECTION_OUTPUT)
    qualification = _read(root / "results/phase2_vetted_corpus" / CONTRACT_OUTPUT)
    ...
    for arm in ARMS:
        ...
        gate = cast(
            Mapping[str, Any],
            cast(Mapping[str, Any], first["summary"])["complete_smoke_gate"],
        )
    ...
    result: dict[str, object] = {
        "schema_version": 1,
        "compatibility_id": "foundry-l3-grpo-signal-qualified-compatibility-v1",
        "decision": "pass",
        ...
        "next_action": (
            "begin counted generic and targeted L3 verifier-GRPO training under the "
            "unchanged frozen schedules"
        ),
    }
...
def run_campaign(root: Path) -> dict[str, object]:
    """Run generic and duplicate, then targeted and duplicate, with no retry path."""

    root = root.resolve()
    environment = _environment(root)
    source_commit = _require_clean_synchronized_main(root)
    raw_root = root / "results/raw/phase2_vetted_corpus/milestone14b_r1/compatibility"
    selection = _read(root / "results/phase2_vetted_corpus" / SELECTION_OUTPUT)
    _verify(selection, "selection_decision_sha256")
    ...
    _write_new(
        root / "results/phase2_vetted_corpus" / OUTPUT_NAME,
        result,
    )
    return result
