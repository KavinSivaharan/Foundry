"""Run one selected-group compatibility smoke through the unchanged official runtime."""
...
RUNTIME_ID = "foundry-l3-grpo-signal-compatibility-wrapper-v1"
...
def run(
    *,
    ...
    """Restrict group selection, then delegate the complete smoke unchanged."""

    root = root.resolve()
    if root != Path(r"C:\Users\Admin\Projects\Foundry").resolve():
        raise ValueError("signal compatibility is attached to the wrong repository")
    if arm not in ARMS or run_index not in (1, 2):
        raise ValueError("signal compatibility arm or run index differs")
    ...
    if (
        ...
        raise ValueError("compatibility selection decision differs")
    ...
    if (
        ...
        raise ValueError("selected arm is not signal-qualified")
    ...
    complete_gate = cast(Mapping[str, Any], summary.get("complete_smoke_gate"))
    final_adapter = cast(Mapping[str, Any], packet.get("final_adapter"))
    expected_group_ids = [selected["task_group_id"], selected["replay_group_id"]]
    if (
        ...
        or cast(int, complete_gate.get("nonzero_variance_group_count", 0)) < 1
        or cast(int, complete_gate.get("nonzero_gradient_group_count", 0)) < 1
        or cast(int, complete_gate.get("policy_update_count", 0)) < 1
        or complete_gate.get("reference_update_count") != 0
        or complete_gate.get("base_update_count") != 0
        ...
        raise RuntimeError("selected official compatibility smoke gate failed")
